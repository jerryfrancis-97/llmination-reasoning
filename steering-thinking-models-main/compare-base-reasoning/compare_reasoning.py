# %%
'''
Modifying the code to compare reasoning abilities for hard math problems
'''
...
from messages.hardmath_messages import messages, labels
...
def get_label_counts(thinking_process, tokenizer, labels, annotate_response=True):
    if annotate_response:
        ...
        Also include a keyword in the format <<>> for each label before the ["end-section"] for the following cases,
        <<M>> -> Memory/ Fact recall, the model is explicitly recalling or restating known facts from memory, internal knownledge base.
    ...
    # Initialize counts for each label-activity pair
    label_activity_counts = {f"{label}-{activity_label}": 0 for label in labels for activity_label in activity_labels}
    ...
    for match in matches:
        ...
        activity_pattern = r'<<(M|AcR)>>'
        activity_matches = re.finditer(activity_pattern, text)
        for activity_match in activity_matches:
            activity_label = activity_match.group(1)
            if activity_label in activity_labels:
                # Count tokens for this label-activity pair
                label_activity_counts[f"{label}-{activity_label}"] += 1
                activity_label_total += 1
...
def process_chat_response(message, model_name, model, tokenizer, labels):
    ...
    return {
        ...
        "label_activity_fractions": label_activity_fractions,
